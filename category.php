				...
				<div class="side">
					<div class="side1">
						<h2> Sections: </h2><br/>
							<?php
								$conn=mysqli_connect("localhost","root","root","craigslist",3306);
								$query_section_category = "select sectionID, section from section where categoryID = $category_id;";
							 	$result = mysqli_query( $conn, $query_section_category);
								while($row = $result->fetch_assoc())
								{
									echo "<a class=\"advertisement_links\" href=\"http://localhost/craiglist/section/".$row["sectionID"]."\">".$row["section"]."</a><br/><br/>";
								}
