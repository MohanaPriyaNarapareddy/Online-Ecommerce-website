		...
		<?php
			if((isset($_SESSION["userid"]))==true)
			{
				...
				else
				{
						...
						$sql="SELECT * FROM advertisements where status='processing';"; 
						...
						$sql="SELECT * FROM jobs_gigs where status='processing';"; 
						$res=mysqli_query($conn,$sql);
						if(mysqli_num_rows($res)>0) 
						{
							while($row=$res->fetch_assoc())
						...
						if($count>1)
						{
							echo "<div class=\"detail_link\">Sorry, there are no new advertisements requested to publish.</div>";
						}
